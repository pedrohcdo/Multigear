...
public class Polygon implements Drawable {
	 ...
	 * @return
	 */
	final public static Polygon createRoundedSquare(final int sides, final int radius) {
		...
		for(double i=Math.PI; i<Math.PI*1.5; i+=detail) {
			final float x = (float)((Math.cos(i) + 1) * radius);
			final float y = (float)(Math.sin(i) * radius * -1) + (sides - radius);
			roundedSquare.addVertice(x, y);
		}
	 ...
	 * @return
	 */
	final public static Polygon createRoundedRectangle(final Vector2 size, final int radius) {
	 ...
	 * @return
	 */
	final public static Polygon createSquare(final int sides) {
	...
	/**
	 * Creates an optimized circular polygon that does not require vertices, 
	 * The vertices not affect the final result.<br>
	 * <b>Note: This method creates the circle in the optimization mode,
	 * drawing a perfect circle without detailing.</b>
	 * 
	 * @param radius Radius
	...
	/**
	 * Creates an optimized ellipse polygon that does not require vertices, 
	 * The vertices not affect the final result.<br>
	 ...
	 * @return Returns the polygon containing the vertices to a circle.
	 */
	final public static Polygon createNgon(final int radius, final int sides) {
		final Polygon circle = new Polygon();
	 ...
	 * @return Returns the polygon containing the vertices to a circle.
	 */
	final public static Polygon createNgon(final Vector2 radius, final int sides) {
		final Polygon circle = new Polygon();
		final double rad = (Math.PI * 2) / sides;
		final double align = -Math.PI / 2;
		for(int i=0; i<sides; i++) {
	...
	final public void addVertice(final Vector2 vector) {
		...
		if((mVertices.position() + 2) > mVertices.limit()) {
			...
			mVertices = GeneralUtils.createFloatBuffer(mVertices.limit() + (int)((mVertices.limit() + 1) * 0.2f) + 10);
