...
import com.org.multigear.communication.tcp.support.listener.Listener;
...
final public class ComManager {
	...
	// Private Variables
	private com.org.multigear.communication.tcp.support.listener.Listener mListener;
		...
		if (message.getCode() == Utils.CODE_INTERFACE_OBJECTMESSAGE) {
			...
			if (code < 0) {
				if (code == OBJECT_CODE_PREPAREPARENTATTRIBUTES) {
					...
					final SupportMessage recvMessage = new SupportMessage(com.org.multigear.communication.tcp.support.SupportMessage.PARENT_PREPAREDATTRIBUTES, clientAttributes);
