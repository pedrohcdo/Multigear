...
import com.org.multigear.communication.tcp.server.Server;
...
public class ServerSupport {
	 ...
	 * Property Createlier.
	 */
	final private class BlockedListener implements com.org.multigear.communication.tcp.server.Listener {
	 ...
	 * Construtor
	 */
	public ServerSupport(final com.org.multigear.communication.tcp.support.ComManager comSupport, final com.org.multigear.mginterface.engine.Manager manager, final String name, final int port) {
	...
	final public void start() {
		if(mStarting | mStarted)
			com.org.multigear.general.utils.KernelUtils.error(mManager.getEngine().getActivity(), "ServerSupport: An error ocurred while calling 'start'. The Server is Starting or has been started..", 0x10);
		mStarting = true;
		mServer.start();
	}
	...
	final public void startWaitForClient() {
		if(!mStarted)
			com.org.multigear.general.utils.KernelUtils.error(mManager.getEngine().getActivity(), "ServerSupport: An error ocurred while calling 'waitForClient'. The Server was not correctly started.", 0x9);
