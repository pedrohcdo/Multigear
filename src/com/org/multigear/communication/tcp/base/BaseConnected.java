...
public class BaseConnected {
	...
	final private class ConducerReceiveThread extends Thread {
		...
		public void run() {
			...
			while(true) {
				...
				// If valid stream
				if (stream != null) {
					Message message = com.org.multigear.communication.tcp.base.Utils.translateSocketMessages(stream);
	...
	final public void sendMessage(final int code, final String message) {
		...
		final String messageSocket = com.org.multigear.communication.tcp.base.Utils.makeSocketMessage(code, message);
	...
	final public void sendMessage(final int code) {
		final String messageSocket = com.org.multigear.communication.tcp.base.Utils.makeSocketMessage(code);
		putMessageToSendThread(messageSocket);
	}
	...
	final public int readMessage(final Message[] out) {
		if (mReadBlocked)
			com.org.multigear.general.utils.KernelUtils.error(mActivity, "BaseConnected: An error occurred while calling the 'ReadMessage' function. The same was blocked, probably by a top object, such as ConSupport.", CODE_ERROR_READMESSAGE_BLOCKED);
		if(out.length == 0)
			return 0;
		int count = 0;
		synchronized(mMessages) {
