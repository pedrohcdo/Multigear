...
import com.org.multigear.communication.tcp.client.ServersList;
...
import com.org.multigear.mginterface.engine.servicessuport.DedicatedServices;
import com.org.multigear.mginterface.engine.servicessuport.SupportThread;
import com.org.multigear.mginterface.engine.servicessuport.DedicatedServices.Database;
 ...
 *         Property Createlier.
 */
@SuppressLint("NewApi") final public class ServicesManager implements com.org.multigear.communication.tcp.support.listener.Listener {
	 ...
	 *
	 */
	final public class FilteredScanResult {
		
		final List<ScanResult> mScanResults;
		final List<ServersList> mServers;
		 ...
		 * @param servers
		 */
		private FilteredScanResult(final List<ScanResult> scans, final List<ServersList> servers) {
			mScanResults = scans;
			mServers = servers;
		}
		 ...
		 * @return
		 */
		final public List<ScanResult> getScanResults() {
			return mScanResults;
		}
		 ...
		 * @return
		 */
		final public ServersList getServersListOn(final int id) {
	...
	final private com.org.multigear.services.Receiver mReceiver;
	final private List<com.org.multigear.services.Message> mMessages;
	final private WifiManager mWifiManager;
	final private ConnectivityManager mConnectivityManager;
	final private com.org.multigear.services.ServicesThreadGroup mServicesGroup;
	final private com.org.multigear.communication.tcp.support.ComManager mComManager;
	final private List<DelayService> mDelayServices;
	
	// Private Variables
	private com.org.multigear.services.Listener mListener;
	private boolean mImmediateAction;
	private List<ScanResult> mScanWifiAccessPointsList;
	private FilteredScanResult mFilteredScanWifiAccessPointsList;
	private com.org.multigear.services.AccessPointsFilter mScanFilter;
	private com.org.multigear.communication.tcp.client.ServersList mServersList;
	 ...
	 * Construtor
	 */
	public ServicesManager(final com.org.multigear.mginterface.engine.Manager manager) {
		mEngine = manager.getEngine();
		mReceiver = new com.org.multigear.services.Receiver(this);
		mMessages = new ArrayList<com.org.multigear.services.Message>();
		...
		mListener = null;
		mServicesGroup = new com.org.multigear.services.ServicesThreadGroup(this);
	...
	/**
	 * Restores the start state. This is valid for
	 * the following services: Wi-fi, Hotspot, 3G.
	 */
	final public ServiceRunnable restoreState() {
		ServiceRunnable runnable = new ServiceRunnable() {
			...
			public void run(ServiceControl serviceControl) {
				...
				for (final WifiConfiguration configuredNetwork : getConfiguredNetworksSafety()) {
					boolean found = false;
					// find
					for(final WifiConfiguration savedNetwork : mDatabase.ConfiguredNetworks) {
				...
				if (mDatabase.HotspotEnabled && !mDatabase.WifiEnabled)
					enableHotspot(mDatabase.HotspotConfiguration, true, serviceControl);
				else {
					setHotspotConfiguration(mDatabase.HotspotConfiguration, serviceControl);
				...
				if (mDatabase.MobileDataEnabled)
					enableMobileData(true, serviceControl);
				else
					disableMobileData(true, serviceControl);
				// Services restored
				postMessage(com.org.multigear.services.Message.SERVICES_STATE_RESTORED);
			}
		};
		// Start Service
		addService(runnable);
		// Return Service for management
		return runnable;
	}
	 ...
	 * 
	 * @param listener
	 *            {@link com.org.multigear.services.Listener}
	 */
	final public void setListener(final com.org.multigear.services.Listener listener) {
	...
	final public void update() {
		...
		synchronized (mMessages) {
			if (mListener != null) {
				for (final com.org.multigear.services.Message message : mMessages)
	...
	final protected void postMessage(final int message) {
		synchronized (mMessages) {
			mMessages.add(new com.org.multigear.services.Message(message));
		}
	}
	 ...
	 * @param object
	 */
	final protected void postMessage(final com.org.multigear.services.Message message) {
		synchronized (mMessages) {
	 ...
	 * Add Service Thread
	 */
	final public void addService(final com.org.multigear.services.ServiceRunnable serviceRunnable) {
	 ...
	 */
	final public void addService(final com.org.multigear.services.ServiceRunnable serviceRunnable, final long delay) {
	 ...
	 */
	final public void removeService(final com.org.multigear.services.ServiceRunnable serviceRunnable) {
		synchronized (mDelayServices) {
	 ...
	 */
	final public void waitForService(final com.org.multigear.services.ServiceRunnable serviceRunnable) {
		// if the service is waiting
		synchronized (mDelayServices) {
			for (final DelayService delayService : mDelayServices)
				if (delayService.mServiceRunnable == serviceRunnable) {
	 ...
	 */
	final private com.org.multigear.services.ServiceRunnable enableWifi(final boolean immediate, final ServiceControl control) {
		// If Immediate Action
		if (immediate) {
			// Wait For Wifi States
			...
			disableMobileData(true, control);
			...
			// Service Runnable
			final com.org.multigear.services.ServiceRunnable serviceRunnable = new ServiceRunnable() {
				...
				public void run(final ServiceControl serviceControl) {
					...
					// If Wifi Enabled ignore this process
					if (isWifiEnabled()) {
						postMessage(com.org.multigear.services.Message.WIFI_ENABLED);
						return;
					}
					// Disable Mobile Data Immediate
					disableMobileData(true, control);
					...
					if(intercepted || mWifiManager.isWifiEnabled())
						...
						postMessage(com.org.multigear.services.Message.WIFI_ENABLE_ERROR);
				}
			};
			// Add Service
	 ...
	 * Note: Need Permissions "CHANGE_WIFI_STATE"
	 */
	final public com.org.multigear.services.ServiceRunnable enableWifi() {
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final private com.org.multigear.services.ServiceRunnable disableWifi(final boolean immediate, final ServiceControl control) {
		// If Immediate Action
		if (immediate) {
			...
			final com.org.multigear.services.ServiceRunnable serviceRunnable = new ServiceRunnable() {
				...
				public void run(final ServiceControl serviceControl) {
					...
					// If Wifi Enabled ignore this process
					if (!isWifiEnabled()) {
						postMessage(com.org.multigear.services.Message.WIFI_DISABLED);
						return;
					}
					...
					if(intercepted || !mWifiManager.isWifiEnabled())
						...
						postMessage(com.org.multigear.services.Message.WIFI_DISABLE_ERROR);
				}
			};
			// Add Service
			addService(serviceRunnable);
	 ...
	 * Note: Need Permissions "CHANGE_WIFI_STATE"
	 */
	final public com.org.multigear.services.ServiceRunnable disableWifi() {
	 ...
	 * @return
	 */
	final private boolean waitForWifiConnetionComplete(final int networkId, final com.org.multigear.services.ServiceControl serviceControl, boolean extraSleep) {
		// Set Last Time
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final private com.org.multigear.services.ServiceRunnable connectTo(final int networkId, final boolean immediate, final ServiceControl control) {
		// If invalid Network
		if (networkId == -1)
			throw new com.org.multigear.services.ServiceException("Could not connect to the network. Id provided is invalid.");
		// If immediate action
		if (immediate) {
			...
			wifiConfiguration.priority = getTopWifiPriority() + 1000;
			...
			//for(WifiConfiguration config : getConfiguredNetworksSafety())
			...
			com.org.multigear.services.ServiceRunnable serviceRunnable = new ServiceRunnable() {
				...
				public void run(ServiceControl serviceControl) {
					...
					// Send Message
					postMessage(com.org.multigear.services.Message.WIFI_CONNECTED);
				}
			};
			// Add Service
			addService(serviceRunnable);
	 ...
	 * 
	 * @return Wifi State
	 *         {@link com.org.multigear.mginterface.engine.servicessuport.DedicatedServices.WIFI_ON}
	 *         or
	 *         {@link com.org.multigear.mginterface.engine.servicessuport.DedicatedServices.WIFI_OFF}
	 */
	final public void scanAccessPoints(final com.org.multigear.services.AccessPointsFilter filter) {
		...
		if(mScanControl.getAndSet(true) == false) {
			...
			if(!intercepted) {
				postMessage(com.org.multigear.services.Message.SCAN_ACCESS_POINT_ERROR);
			} else {
				// Set Filter
				mScanFilter = filter;
				// Request filter
	...
	final protected void scanAccessPointsComplete() {
		...
		if(mScanControl.getAndSet(true) == false) {
			...
			if (mScanFilter != null) {
				...
				/** Runnable */
				final com.org.multigear.services.ServiceRunnable serviceRunnable = new com.org.multigear.services.ServiceRunnable() {
					...
					@Override
					public void run(ServiceControl serviceControl) {
						mFilteredScanWifiAccessPointsList = filterScanResult(mWifiManager.getScanResults(), serviceControl);
						if(mFilteredScanWifiAccessPointsList == null) {
							mScanWifiAccessPointsList = null;
							postMessage(com.org.multigear.services.Message.SCAN_ACCESS_POINT_ERROR);
						} else {
							mScanWifiAccessPointsList = mFilteredScanWifiAccessPointsList.mScanResults;
							postMessage(com.org.multigear.services.Message.SCAN_ACCESS_POINT_COMPLETED);
						}
						// Release lock
						mScanControl.set(false);
					}
	 ...
	 */
	@SuppressLint("NewApi") 
	final private FilteredScanResult filterScanResult(final List<ScanResult> scanResultList, final com.org.multigear.services.ServiceControl serviceControl) {
		// Create Filtered Scan Result List
		final List<ScanResult> scans = new ArrayList<ScanResult>();
		final List<ServersList> servers = new ArrayList<ServersList>();
		...
		for (final ScanResult scanResult : scanResultList) {
			...
			if (mScanFilter.isValidScanResult(scanResult)) {
				...
				if (preWifiConfiguration != null) {
					...
					Log.d("LogTest", "Founded");
					//logWifiConfiguration("CONNECTION", preWifiConfiguration);
				  // Not configurated
				} else
				...
				if (netId != -1) {
					...
					if (waitForWifiConnetionComplete(netId, serviceControl, false)) {
						...
						mComManager.createClient("TestClient").listServers(0);
		...
		return new FilteredScanResult(scans, servers);
	}
	...
	final public List<ScanResult> getScanResult() {
		if (mScanWifiAccessPointsList == null)
			throw new com.org.multigear.services.ServiceException("Was not possible to capture the scan list. Because no research has been done previously.");
	 ...
	 * @return
	 */
	final public FilteredScanResult getFilteredScanResult() {
		if (mFilteredScanWifiAccessPointsList == null)
			throw new com.org.multigear.services.ServiceException("Was not possible to capture the scan list. Because no research has been done previously.");
		return mFilteredScanWifiAccessPointsList;
	}
	...
	final private boolean enableHotspot(final ServiceControl control, final WifiConfiguration configuration) {
		...
		try {
			...
			if (result) {
				...
				boolean intercepted = SafetyLock.lock(8000, new Interception() {
					...
					public boolean onIntercept() {
						...
						// TODO Auto-generated method stub
						return enabled || (control != null && control.isEndService()) || Thread.currentThread().isInterrupted();
					}
				});
	...
	final private ServiceRunnable enableHotspot(final WifiConfiguration hotspotConfiguration, final boolean immediate, final ServiceControl control) {
		...
		if (immediate) {
			...
			final com.org.multigear.services.ServiceRunnable serviceRunnable = new com.org.multigear.services.ServiceRunnable() {
				 ...
				 */
				@Override
				public void run(final com.org.multigear.services.ServiceControl serviceControl) {
					// Enabled state
					boolean enabled = enableHotspot(serviceControl, hotspotConfiguration);
					// Lock
					if (enabled)
						postMessage(com.org.multigear.services.Message.HOTSPOT_ENABLED);
					else
						postMessage(com.org.multigear.services.Message.HOTSPOT_ENABLE_ERROR);
				}
			};
			// Add Service
			addService(serviceRunnable);
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final public com.org.multigear.services.ServiceRunnable enableHotspot(final WifiConfiguration hotspotConfiguration) {
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final private com.org.multigear.services.ServiceRunnable enableHotspot(final String name, final String password, final boolean immediate, final ServiceControl control) {
		// Immediate Action
		if (immediate) {
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final public com.org.multigear.services.ServiceRunnable enableHotspot(final String name, final String password) {
	 ...
	 */
	final private void logWifiConfiguration(final String tag, final WifiConfiguration config) {
		Log.d("LogTest", "Wifi Configuration: " + tag);
		Log.d("LogTest", "	SSID:" + config.SSID);
		Log.d("LogTest", "	BSSID:" + config.BSSID);
		Log.d("LogTest", "	HIDDEN_SSID:" + config.hiddenSSID);
		Log.d("LogTest", "	NET_ID:" + config.networkId);
		Log.d("LogTest", "	PRE_SHARED_KEY:" + config.preSharedKey);
		Log.d("LogTest", "	PRIORITY:" + config.priority);
		Log.d("LogTest", "	STATUS:" + config.status);
		Log.d("LogTest", "	WEPTX_KEY_INDEX:" + config.wepTxKeyIndex);
		
		for(final String key : config.wepKeys)
			Log.d("LogTest", "		-> WEP_KEY:" + key);
		
		Log.d("LogTest", "	ALLOWED_AUTH_ALGORITHMS:");
		for(int i=0; i<config.allowedAuthAlgorithms.length(); i++)
			Log.d("LogTest", "		" + config.allowedAuthAlgorithms.get(i));
		
		Log.d("LogTest", "	ALLOWED_GROUP_CIPHERS:");
		for(int i=0; i<config.allowedGroupCiphers.length(); i++)
			Log.d("LogTest", "		" + config.allowedGroupCiphers.get(i));
		
		Log.d("LogTest", "	ALLOWED_KEY_MANAGEMENT:");
		for(int i=0; i<config.allowedKeyManagement.length(); i++)
			Log.d("LogTest", "		" + config.allowedKeyManagement.get(i));
		
		Log.d("LogTest", "	ALLOWED_APIRWISE_CIPHERS:");
		for(int i=0; i<config.allowedPairwiseCiphers.length(); i++)
			Log.d("LogTest", "		" + config.allowedPairwiseCiphers.get(i));
		
		Log.d("LogTest", "	ALLOWED_PROTOCOLS:");
		for(int i=0; i<config.allowedProtocols.length(); i++)
			Log.d("LogTest", "		" + config.allowedProtocols.get(i));
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final private com.org.multigear.services.ServiceRunnable enableHotspot(final String name, final boolean immediate, final ServiceControl control) {
		// Immediate Action
		if (immediate) {
			// Create Configuration
			final WifiConfiguration hotspotConfiguration = new WifiConfiguration();
			...
			final com.org.multigear.services.ServiceRunnable serviceRunnable = new com.org.multigear.services.ServiceRunnable() {
				...
				public void run(final com.org.multigear.services.ServiceControl serviceControl) {
					...
					final WifiConfiguration hotspotConfiguration = new WifiConfiguration();
					hotspotConfiguration.SSID = name;
					hotspotConfiguration.BSSID = null;
					hotspotConfiguration.hiddenSSID = false;
					hotspotConfiguration.networkId = -1;
					hotspotConfiguration.preSharedKey = null;
					hotspotConfiguration.priority = 0;
					hotspotConfiguration.status = 0;
					for(int i=0; i<hotspotConfiguration.wepKeys.length; i++)
						hotspotConfiguration.wepKeys[i] = null;
					hotspotConfiguration.allowedAuthAlgorithms.clear();
					hotspotConfiguration.allowedGroupCiphers.clear();
					hotspotConfiguration.allowedKeyManagement.set(0);
					hotspotConfiguration.allowedPairwiseCiphers.clear();
					hotspotConfiguration.allowedProtocols.clear();
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final public com.org.multigear.services.ServiceRunnable enableHotspot(final String name) {
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final private com.org.multigear.services.ServiceRunnable disableHotspot(final boolean immediate, final ServiceControl control) {
		// Immediate Action
		if (immediate) {				
			// Wait for hotspot state
			waitForHotspotState(control);
			...
			final com.org.multigear.services.ServiceRunnable serviceRunnable = new com.org.multigear.services.ServiceRunnable() {
				...
				public void run(final com.org.multigear.services.ServiceControl serviceControl) {
					...
					// Return if hotspot enabled
					if (!isHotspotEnabled()) {
						postMessage(com.org.multigear.services.Message.HOTSPOT_DISABLED);
						return;
					}
					...
					// Disable error
					postMessage(com.org.multigear.services.Message.HOTSPOT_DISABLE_ERROR);
				}
			};
			// Add Service
			addService(serviceRunnable);
	...
	final private void setMobileDataAndroidVersion2_2(final boolean enabled, final ServiceControl serviceControl) {
		...
		if (!enabled) {
			...
			SafetyLock.lock(5000, new Interception() {
				...
				@Override
				public boolean onIntercept() {
					if(serviceControl != null && serviceControl.isEndService() || Thread.currentThread().isInterrupted())
						return true;
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final private com.org.multigear.services.ServiceRunnable enableMobileData(final boolean immediate, final ServiceControl control) {
		// Immediate Action
		if (immediate) {
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final public com.org.multigear.services.ServiceRunnable enableMobileData() {
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final private com.org.multigear.services.ServiceRunnable disableMobileData(final boolean immediate, final ServiceControl control) {
		// Immediate Action
		if (immediate) {
			// Enable Mobile Data
			setMobileData(false, control);
			// No Service Launched
			return null;
		} else {
			/** Runnable */
	 ...
	 *         will be returned to instantiate the service launched.
	 */
	final public com.org.multigear.services.ServiceRunnable disableMobileData() {
		return disableMobileData(mImmediateAction, null);
	}
