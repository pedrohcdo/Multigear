...
final public class InstallManager {
	...
	final private class Handler {
		 ...
		 * @param code
		 */
		private Handler(final int code, final com.org.multigear.mginterface.scene.Installation installation, int extra) {
			mCode = code;
			...
			switch (mCode) {
				...
				case UNINSTALL:
					...
					if (mInstallationList.remove(mInstallation)) {
					  ...
					  com.org.multigear.general.utils.KernelUtils.error(mRoom.getEngine().getActivity(),
	...
	// existente
	final private List<com.org.multigear.mginterface.scene.Installation> mInstallationList;
	// Lista de instalações que estao aguardando um determinado tempo
	final private List<com.org.multigear.mginterface.scene.DelayIU> mInstallDelay;
	// Lista de desinstalações que estao aguardando um tempo
	final private List<com.org.multigear.mginterface.scene.DelayIU> mUninstallDelay;
	// Lista de desinstalações que estaão ocrrendo no momento
	final private List<com.org.multigear.mginterface.scene.Installation> mUninstallingInstances;
	 ...
	 * Constutor
	 */
	protected InstallManager(final com.org.multigear.mginterface.scene.Scene room) {
		mRoom = room;
		mInstallationList = new ArrayList<com.org.multigear.mginterface.scene.Installation>();
		mInstallDelay = new ArrayList<com.org.multigear.mginterface.scene.DelayIU>();
		mUninstallDelay = new ArrayList<com.org.multigear.mginterface.scene.DelayIU>();
		mUninstallingInstances = new ArrayList<com.org.multigear.mginterface.scene.Installation>();
	...
	final private int firstPrepare(final Installation installation) {
		// Link to Engine
		if (installation instanceof com.org.multigear.mginterface.scene.Scene) {
			com.org.multigear.mginterface.scene.Scene room = (com.org.multigear.mginterface.scene.Scene) installation;
			if (!room.hasEngine())
				((com.org.multigear.mginterface.scene.Scene) installation).setEngine(mRoom.getEngine());
		}
	 ...
	 * @param installation
	 */
	final private void addProc(final int code, final com.org.multigear.mginterface.scene.Installation installation, int extra) {
	 ...
	 *            Installation
	 */
	final protected void install(final com.org.multigear.mginterface.scene.Installation installation) {
		if (mInstallationList.contains(installation))
			com.org.multigear.general.utils.KernelUtils.error(mRoom.getEngine().getActivity(), "InstallSupport: An error occurred while installing an object. The same is already installed.", 0xE);
	 ...
	 *            Installation
	 */
	final protected void uninstall(final com.org.multigear.mginterface.scene.Installation installation) {
	 ...
	 */
	final protected void installDelayed(final com.org.multigear.mginterface.scene.Installation installation, final long delay) {
		mInstallDelay.add(new com.org.multigear.mginterface.scene.DelayIU(mRoom.getThisTime(), delay, installation));
	}
	
	/**
	 * Uninstall with delayed
	 */
	final protected void uninstallDelayed(final com.org.multigear.mginterface.scene.Installation installation, final long delay) {
		mUninstallingInstances.add(installation);
		mUninstallDelay.add(new com.org.multigear.mginterface.scene.DelayIU(mRoom.getThisTime(), delay, installation));
	}
	 ...
	 * 
	 * @param installation
	 *            {@link com.org.multigear.mginterface.scene.Installation}
	 * @return True if has uninstalling.
	 */
	final protected boolean hasUninstalling(final com.org.multigear.mginterface.scene.Installation installation) {
	 ...
	 * Arquiva as texturas
	 */
	final protected void cache(final com.org.multigear.mginterface.graphics.opengl.texture.Loader textureLoader) {
		mLifeStep = 3;
		for (int index = 0; index < mInstallationList.size(); index++)
	...
	final protected void updateManager() {
		if (mInstallDelay.size() > 0) {
			Iterator<com.org.multigear.mginterface.scene.DelayIU> itr = mInstallDelay.iterator();
			while (itr.hasNext()) {
				com.org.multigear.mginterface.scene.DelayIU delayIU = itr.next();
		...
		if (mUninstallDelay.size() > 0) {
			Iterator<com.org.multigear.mginterface.scene.DelayIU> itr = mUninstallDelay.iterator();
			while (itr.hasNext()) {
				com.org.multigear.mginterface.scene.DelayIU delayIU = itr.next();
				if (delayIU.isTimesUp(mRoom.getThisTime())) {
